...
void usage() {
    ...
    printf("sample: send-arp wlan0 192.168.10.2 192.168.10.1 192.168.10.3 192.168.10.1\n");
}
...
int main(int argc, char* argv[]) {
    ...
    u_int8_t* attacker_mac = get_attacker_mac(dev).operator uint8_t*();
    std::string attacker_ip = get_attacker_ip(dev).operator std::string();

    for (int i = 2; i < argc; i += 2) {
        std::string sender_ip = argv[i];
        std::string target_ip = argv[i + 1];

        u_int8_t* sender_mac = get_sender_mac(handle, attacker_mac, attacker_ip, sender_ip).operator uint8_t*();
...
Mac get_sender_mac(pcap_t* handle, u_int8_t* attacker_mac, std::string attacker_ip, std::string sender_ip) {
    ...
    struct EthArpPacket* check_header = nullptr;
    ...
    while (true) {
        ...
        if (check_header->eth_.type() == 0x0806) { // ARP packet
            break;
        }
    }
